...
class SGCN(nn.Module):
    ...
    def forward(self, node_features, edge_index):
        ...
        return x
class GaitHyperGraph(nn.Module):
    def __init__(self, a_dim, v_dim, l_dim, n_dim, nlayers, nhidden, nclass, dropout, lamda, alpha, variant, return_feature, use_residue, 
                new_graph='full',n_speakers=2, modals=None, use_speaker=True, use_modal=False, num_L=3, num_K=4):
        super(GaitHyperGraph, self).__init__()
    ...
    def forward(self, a, v, l, agent_gnn, dia_len, qmask, epoch):
        ...
        out = self.reverse_features1(dia_len, out)
        return agent_gnn, out 
        ...
        for i in dia_len:
            ...
            for _ in range(i+4):
                if _ < 4:
                    ...
                    edge_count = edge_count + 1
                    index2 = index2 + [edge_count]*2                   
                edge_count = edge_count + 1 # edge_count = i + 4
            if node_count == 0:
                ll = l[0:0+i]
                aa = a[0:0+i]
                vv = v[0:0+i]
                gg = agent[0:0+i]# torch.Size([i, 512])
                features = torch.cat([ll,aa,vv,gg],dim=0)# torch.Size([i*4, 512])
                temp = 0+i
            else:
                ll = l[temp:temp+i]
                aa = a[temp:temp+i]
    ...
    def reverse_features1(self, dia_len, features):
        l=[]
        a=[]
        v=[]
        g=[]
...
class HyperGCN(nn.Module):
    ...
    def forward(self, a, v, l, dia_len, qmask, epoch):
        ...
        out = out + gnn_out # torch.Size([2247, 512]) torch.Size([2247, 512])
        if self.use_residue:
            out = torch.cat([gnn_features, out], dim=-1)
        agent = self.reverse_features(dia_len, out) # torch.Size([749, 512 * 3])
        agent = self.fc2(agent)   # torch.Size([749, 512 * 3])->  torch.Size([749, 512])
    ...
    def create_hyper_index(self, a, v, l, agent, dia_len, modals):
        ...
        hyperedge_type1 = []
        for i in dia_len:
            nodes = list(range(i*num_modality)) # 构建i*4个点
            nodes = [j + node_count for j in nodes]
            nodes_l = nodes[0:i*num_modality//4]
            nodes_a = nodes[i*num_modality//4:i*num_modality*2//4]
            nodes_v = nodes[i*num_modality*2//4:i*num_modality*3//4]
